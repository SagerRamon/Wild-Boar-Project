...
Miriam <- recurr_layer_plot(ws_sf_miriam, underl_map, 100)

ws_sf_olga <- ws_sf %>% filter(TierID %in% c(36)) %>% select(DatetimeUTC, TierID)
Olga <- recurr_layer_plot(ws_sf_olga, underl_map, 100)
```
...
Franz <- recurr_layer_plot(ws_sf_franz, underl_map, 100)

ws_sf_amos <- ws_sf %>% filter(TierID %in% c(48)) %>% select(DatetimeUTC, TierID)
Amos <- recurr_layer_plot(ws_sf_amos, underl_map, 100)
