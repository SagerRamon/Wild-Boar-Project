...
new_agg <- st_join(grid_area, by_cell, by = ID)
...
```{r}
# c(10, 22, 36, 40, 48))
ws_sf_dt <- ws_sf %>% filter(TierID %in% c(10, 22, 36, 40, 48)) %>% select(DatetimeUTC, TierID)
...
layer_22 <- recurr_layer_plot(ws_sf_dt, underl_map, 150)

ws_sf_dt <- ws_sf %>% filter(TierID %in% c(36)) %>% select(DatetimeUTC, TierID)
layer_36 <- recurr_layer_plot(ws_sf_dt, underl_map, 150)
```
